...
- `main.py`
  - 主入口
  - 串联整轮周期流程：首轮校时、首轮对齐、初始化、识别、监听主站读取、休眠唤醒
...
- `modbus.py`
  ...
  - 负责：
    ...
    - 监听主站请求
    - 校验请求帧并回发响应帧
    - 主动读取 RTU 时间并校准 Jetson 系统时间和 `/dev/rtc0`
...
- 程序启动后，不立即进入业务
- 先打开 Modbus 串口，主动读取 RTU 时间并校准 Jetson 系统时间和 `/dev/rtc0`
- 首轮校时完成或失败后，关闭临时串口
- 再按校时后的系统时间对齐到下一个整 5 分钟时间点
- 对齐完成后开始第一轮业务
...
2. 初始化阶段
   ...
   - 初始化内容：
     ...
     - RTSP 视频流
     - 推理引擎
   - 初始化成功后立即进入识别，不再强制等待到 `T+15s`

3. 识别阶段
   - 初始化完成后立即执行，不再强制等待到 `T+15s`
   - 当前先执行 RTSP 取帧
   - 成功取帧后：
     - 现在默认走模拟水位
...
- 周期长度为 300 秒，即整 5 分钟
- 前 15 秒是初始化最大窗口，成功后会提前结束初始化
- `MODBUS_LISTEN_START_SECONDS` 当前不再由主流程使用，通信监听改为寄存器写入后立即开始
- 通信监听最晚到本轮第 60 秒结束
...
- 时间起始寄存器：`0x0042`
- 时间寄存器数量：`0x0004`
- 请求帧格式：`FE 04 00 42 00 04 CRC低字节 CRC高字节`
- 其中 CRC 是根据前 6 个字节计算得到的 Modbus RTU 校验码，不是要发送的字母
- `0x0042`：年月，BCD 码，年在前
- `0x0043`：日时，BCD 码，日在前
- `0x0044`：分秒，BCD 码，分在前
- `0x0045`：默认 `0000`
...
- 程序启动后，首轮整 5 分钟对齐前执行一次校时
- 每轮初始化阶段，Modbus 串口打开后执行一次校时
- 当前校时频率为：程序启动时 1 次，之后每天 `00:00` 和 `12:00` 各 1 次
- `24:00` 按实际时间表示为次日 `00:00`
- 校时成功后，以 RTU 返回时间设置 Jetson 系统时间，并同步写入 `/dev/rtc0`
- 校时失败只记录错误，不阻断本轮后续识别、通信和休眠流程
- 校时时 Jetson 临时作为 Modbus 主站读取 RTU；水位通信阶段恢复为 Modbus 从站
